...
# Code from dom_migration_byAgeGroup_20240213  (originally derived from 20230712.R)

# MIGRATION FROM LONDON BY SYA
...
input_london_dom_mign_out_SYA <- read_rds("C:/Migration/Migration_R/DATA/Domestic/copied_from_Qdrive_20240213/origin_destination_lad_2002_2022.rds") %>%
...
write.csv(input_london_dom_mign_out_SYA, paste0(data_dir, "summarised_london_origin_SYA.csv"))


# ---------------------------------------------------------------------------------

### MIGRATION TO LONDON BY SYA
...
input_london_dom_mign_in_SYA <-  read_rds("C:/Migration/Migration_R/DATA/Domestic/copied_from_Qdrive_20240213/origin_destination_lad_2002_2022.rds") %>%
  filter(substr(gss_in,1,3) %in% c("E09")) %>% # "in" is destination
  filter(!(substr(gss_out,1,3) %in% c("E09"))) %>%   # "out" is origin
  ...
  group_by(gss_out, year, age) %>% 
  mutate(across(where(anyNA), ~ replace_na(., 0))) %>% # replace NAs with zeros
  summarise(sumvaluein = sum(value), .groups = 'drop_last') %>%
  mutate(sumvalueinK = sumvaluein/1000) %>% 
    data.frame()  

write.csv(input_london_dom_mign_in_SYA, paste0(data_dir, "summarised_london_destination_SYA.csv"))
...
### NET MIGRATION FOR LONDON BY SYA
...
##  Plot net migration over time, by age 
Net_flows_age_group_update <- 
  ggplot() +
  theme_gla() +
  geom_line(data = sum_London_dom_mign_net_SYA, aes(x = year, y = totalvalueK_net, group = age, colour = agegroup), size = 2) +
  geom_label_repel(data = sum_London_dom_mign_net_SYA, aes(x = year, y = totalvalueK_net, label = label, colour = agegroup), nudge_x = 1) +
...
Net_flows_age_group_update_rep <- 
  ggplot() +
  theme_gla() +
...
ggsave (filename = "C:/Families/Charts_for_Daryl/22_C_DomMig_Net_flows_age_group_update_noK.svg",
         plot = Net_flows_age_group_update_rep,
         device = "svg",
         dpi=600,
         width = 9,
         height = 5.56,
